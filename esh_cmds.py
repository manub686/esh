...
LOW_INIT_LEVEL = 0
DEBS_INIT_LEVEL = 4		#debug sessions have been created
HIGH_INIT_LEVEL = 5		#targets have been connected
...
def cmd_load(jsproc, options, args):
  ...
  esh_jsutil.exec_jscmd(jsproc, esh_jsutil.code_load_program(session_id, prog))
  return 0
...
def cmd_run(jsproc, options, args):
  ...
  esh_jsutil.exec_jscmd(jsproc, esh_jsutil.code_run_target(session_id))
  return 0
...
def cmd_reset(jsproc, options, args):
  if not assert_init_level(DEBS_INIT_LEVEL, options):
    return -1
...
def evminit2(jsproc, options, args):
  ...
  if ((not options.cleanup) and options.gui):
    print 'Starting CCS'
    esh_jsutil.exec_jscmd(jsproc, esh_jsutil.code_start_ccs())
...
def evminit3(jsproc, options, args):
  print 'evminit3: Starting debug server'
...
def evminit4(jsproc, options, args):
  print 'evminit4: Starting debug sessions'
  r3 = []
...
def evminit5(jsproc, options, args):
  print 'evminit5: Connecting debug sessions'
  r4 = []
  for session_id in options.debugSessions:
...
def evmshutdown4(jsproc, options, args):
  ...
  if ((not options.cleanup) and options.gui):
    print 'Stopping CCS'
    esh_jsutil.exec_jscmd(jsproc, esh_jsutil.code_stop_ccs())
    time.sleep(10)
